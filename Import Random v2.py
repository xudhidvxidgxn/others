    ...
    if a < chan:
        if 확천 == True:
            if x == "캐릭터":
                if 명함:
                    p += 1
                    recordTxt += f"({p}돌)" if p < 6 else "(풀돌)"
                else:
                    명함 = True
                    recordTxt += "(명함)"
            else:
                if 전광:
                    q += 1
                    recordTxt += f"({q}재)" if q < 5 else "(풀재)"
                else:
                    전광 = True
                    recordTxt += "(전광)"
...
def record(a):
    ...
    recording = {"확": 확천, "stack": stack, "astack": astack, "character": a}
...
def recordprint():
    ...
    for i in range(len(rec)):
        print(
            f"{'0'*(len(str(len(rec)-1))-len(str(i+1)))}{i+1} : {'0' if len(str(rec[i]['stack'])) == 1 else ''}{rec[i]['stack']}회차 ({'0'*(len(str(rec[len(rec)-1]['astack']))-len(str(rec[i]['astack'])))}{rec[i]['astack']}회차) | {rec[i]['character']}"
        )
    for i in rec:
        if "픽업" in i["character"] or "광추" in i["character"]:
            pic += 1
        else:
            upic += 1
...
def c(a=""):
    ...
    stack, astack, p, q, 확천, 명함, 전광, 풀돌, 풀재 = (
        0,
        0,
        0,
        0,
...
def maingame():
    ...
    while True:
        if jamMode:
            c()
            x = input(
                f"어떤 뽑기? (성옥 '{jam}' 개 장전됨) ({jam//160}뽑) || 1. 캐릭터 | 2. 광추 | : "
            )
            b = input(
                f"--- {'광추' if x == "2" or x == "광추" else '캐릭터'} 뽑기 ---\n1. {'풀재?' if x == "2" or x == "광추" else '풀돌?'} | 2. 명함? | : "
            )
